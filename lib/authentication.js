...
//find the user based on the userId in the session
  ...
  .then((user) => {
    if(!user) {
      ...
      return req.session.regenerate(() => res.unauthorizwd());
    }
    //set the userId back on the sessionSecret
    req.session.userId = user.id;
    //set the whole user object to the request object//so we can use the user's details in our controllers
    req.user = user;
